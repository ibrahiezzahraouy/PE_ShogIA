...
class PromotionWindow(Toplevel):
    ...
    def disp(self):
        ...
        self.question = Label(self, text = "Promouvoir ?")
        self.question.pack(padx = 5, pady = 15, side = TOP)
                                
        self.buttonYes = Button(self, text = "Oui", command = self.PromoteYes)
        self.buttonYes.pack(padx = 30, pady = 5, side = LEFT)
                                
        self.buttonNo = Button(self, text = "Non", command = self.destroy)
        self.buttonNo.pack(padx = 30, pady = 5, side = RIGHT)
...
class SideWindow(Toplevel):
    ...
    def disp(self):
        ...
        self.question = Label(self, text = "Choisissez un côté")
        self.question.pack(padx = 5, pady = 15, side = TOP)
                                
        self.buttonYes = Button(self, text = "Blanc", command = self.white)
        self.buttonYes.pack(padx = 30, pady = 5, side = RIGHT)
                                
        self.buttonNo = Button(self, text = "Aléatoire", command = self.randomSide)
        self.buttonNo.pack(padx = 30, pady = 5, side = RIGHT)
        
        self.buttonNo = Button(self, text = "Noir", command = self.black)
        self.buttonNo.pack(padx = 30, pady = 5, side = RIGHT)
...
class Board(Canvas):
    ...
    def __init__(self,parent,pieces):
        ...
        Canvas.__init__(self, master = parent, width = self.width, height = self.height, bg = "#FFE4B5", highlightthickness = 0)
    ...
    def highlight_reachable_squares(self,piece): 
        ...
        if piece == None:
            ...
            for move in self.parent.gameState.legal_moves:
...
class Window(Tk):
    ...
    def __init__(self, mode, level):
        ...
        self.shownBoard = Board(self,self.gameState.board) #Displaying the board
        ...
        self.whiteCaptured = Captured(self,self.gameState.es_pieces["b"]) 
        self.blackCaptured = Captured(self,self.gameState.es_pieces["w"])
        ...
        if self.mode == "Contre l'IA":
            ...
            self.after(200,self.AIPlay) #Timer to allow a bit of time for the board to draw itself
        
    
    def destroy(self):
        Tk.destroy(self)
        self.gameState = Game_state()
        if self.mode == "Contre l'IA" and self.driver != None:
            self.driver.quit()
            self.driver = None
    ...
    def AIPlay(self):
        ...
        if self.gameState.playing_side != self.chosenSide:
            
            print("Processing, side : ", self.gameState.playing_side)
            (_,movelist,usedriver) = ABPruning.alphaBetaPruning(ABPruning.Node(self.gameState, self.gameState.playing_side, len(self.moveHistory), self.moveHistory),self.level, driver = self.driver)
            print("Done")
            if not usedriver:
                self.driver = None
            move = movelist[0]
            self.moveHistory.append(move)
            self.gameState.update(move)
    ...
    def checkmate(self):
        if self.gameState.finished():
            if self.gameState.playing_side == "w":
                msg.showinfo("Victoire !","Le joueur noir gagne")
            else:
                msg.showinfo("Victoire !","Le joueur blanc gagne")
    ...
    def click(self,event): 
        ...
        if self.selectedPiece == None: 
            ...
            self.selectedPiece = self.gameState.board[i,j]
            ...
            if oldi == i and oldj == j: 
                ...
                if s == self.selectedPiece.side: 
                    ...
                    if self.selectedPiece.pos == None:
                            ...
                            if prefix.upper() + "*" + id2str((i,j)) in self.gameState.legal_moves: 
                                #if you can drop there
                                drop = prefix.upper() + "*" + id2str((i,j))
                                self.gameState.update(drop) #Giving game_engine the needed info
                                self.moveHistory.append(drop)
                            else: 
                                #if you can't drop there
                                self.shownBoard.highlight_reachable_squares(None)
                                msg.showwarning("Déplacement illégal","Vous ne pouvez pas parachuter une pièce ici")
                    
                    elif id2str(self.selectedPiece.pos) + id2str((i,j)) in self.gameState.legal_moves or id2str(self.selectedPiece.pos) + id2str((i,j)) + '+' in self.gameState.legal_moves: 
                            #Else, if the move is valid
                            #Case of a move
                            print(id2str(self.selectedPiece.pos) + id2str((i,j)))
                        ...
                        #Wrong move
                        self.shownBoard.highlight_reachable_squares(None)
                        msg.showwarning("Déplacement illégal","Cette pièce ne peut pas faire ce déplacement")
                        self.selectedPiece = None
                        
                else: 
                    #Wrong side
                    self.shownBoard.highlight_reachable_squares(None)
                    msg.showwarning("Déplacement illégal","Vous jouez du mauvais côté")
                    self.selectedPiece = None
        print(id2str((i,j)))
        ...
        self.checkmate()
        if self.mode == "Contre l'IA":
            self.after(200,self.AIPlay)
            
#wind = Window("Contre l'IA",1)
#wind.mainloop()