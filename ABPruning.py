...
def cutDrops(moves): #reduce the number of drops because it heightens the branching factor A LOT
    for i in moves:
        if '*' in i:
...
def alphaBetaPruning(Node, depth, alpha = -100000, beta = 100000, maximizingPlayer = True, driver = None):

    if depth == 0 or Node.gameState.finished():
        return (Node.value,[],False)
    
    if Node.depth < 12 and driver != None: #Opening book
        ...
        if negamax[:4] != 'tion':
            ...
            movelist = movelist[3:]
            moveNumber = 0
            if random.random() < 0.8: #just to not make it always pick the very best move, otherwise it's too predictable
                moveNumber += 1
            if movelist != []: #sometimes the website itself bugs so it's to avoid that
                return (negamax,[movelist[moveNumber%len(movelist)]],True)
    ...
    if maximizingPlayer:
        ...
        for son in Node.sons:
            (v2,mvl,usedriver) = alphaBetaPruning(son, depth - 1, alpha, beta, False)
            if v < v2:
                v = v2
            ...
            if alpha >= beta:
                break
        return (v,[move] + movelist,False)
        
    else:
        v = 100000
        for son in Node.sons:
            (v3,mvl,usedriver) = alphaBetaPruning(son, depth - 1, alpha, beta, True)
